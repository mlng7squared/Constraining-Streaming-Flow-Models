...
def Rollout_WIP(
        ...
        alpha: float = 5.0,
        ## Strength of penalty near the wall -- TUNE! TODO!
        p: float = 2.0,
        ## How fast the penalty grows as you approach the wall (decay shape)
    ...
    with tqdm(total=max_steps, desc="Eval PushTStateEnv") as pbar:
        while not done:
            ...
            for i in range(len(action)):
                ...
                #TODO
                x_cur = np.asarray(obs_deque[-1][:2], dtype=np.float64)  # agent pos (2,)
                a_prop = np.asarray(action[i][:2], dtype=np.float64) # proposed action (2,)
                ...
                # stepping env
                obs, reward, done, _, info = env.step(action_step[i]) # the modified action_step
                # save observations
