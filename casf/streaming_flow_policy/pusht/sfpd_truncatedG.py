...
from streaming_flow_policy.pusht.truncated_gaussian import add_truncated_gaussian_noise
...
class StreamingFlowPolicyDeterministic (Policy):
    ...
    def TransformTrainingDatum(self, datum: Dict[str, np.ndarray]) -> Dict[str, np.ndarray]:
        ...
        # TODO -- truncate the noise/gaussian

        # x = x + self.sigma.item() * np.random.randn(*x.shape)  # (1, ACTION_DIM)
        lo, hi = -1, 1
        x = add_truncated_gaussian_noise(x, self.sigma, lo, hi)
    ...
    def Loss(self, batch: Dict[str, Tensor]) -> Tensor:
        ...
        # # TODO-1 metric-weighted loss
        # # --- squeeze time dim for convenience ---
        # x_s   = x.squeeze(1)        # (B,2)
        # vp_s  = v_pred.squeeze(1)   # (B,2)
        # v_s   = v.squeeze(1)        # (B,2)
        # err   = vp_s - v_s          # (B,2)

        # # --- obstacle-aware metric (circle example) ---
        # center = torch.tensor([0.0, -2.0], device=x_s.device)  # EDIT to your obstacle
        # radius = 1.0                                           # EDIT to your obstacle

        # phi, n = self.sdf_circle(x_s, center, radius)               # (B,), (B,2)
        # M, w    = self.metric_M_from_sdf(phi, n, alpha=5.0, p=2.0, eps=1e-2, w_max=1e3)  # tune

        # # metric-weighted data fit: (err^T M err), averaged
        # # -> (B,1,2) @ (B,2,2) @ (B,2,1) -> (B,)
        # err_col = err.unsqueeze(-1)                             # (B,2,1)
        # quad = torch.matmul(torch.matmul(err_col.transpose(1,2), M), err_col).squeeze(-1).squeeze(-1)  # (B,)
        # loss_metric = quad.mean()

        # # --- barrier: penalize motion into the obstacle when close ---
        # d_safe = 0.5  # start penalizing within this band (edit)
        # gate = torch.clamp((d_safe - phi) / d_safe, min=0.0, max=1.0)  # (B,)
        # into = torch.relu(-(vp_s * n).sum(dim=-1))                      # (B,)  only if n·vp_s < 0
        # loss_barrier = (gate * w * into**2).mean()
        ...
        # loss = λ_mse * loss_mse + λ_metric * loss_metric + λ_barrier * loss_barrier
