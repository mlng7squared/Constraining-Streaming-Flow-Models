...
def add_truncated_gaussian_noise(x, sigma, lo, hi):
    """
    x:    (..., D) numpy array or scalar (current state)
    sigma: float or (..., D) std dev
    lo,hi: float or (..., D) per-dim bounds
    returns: same type as x, guaranteed elementwise in [lo, hi]
    """
    
    # --- detect input type & make torch tensors on CPU with matching dtype ---
    x_is_numpy = isinstance(x, np.ndarray)
    x_t = torch.from_numpy(x) if x_is_numpy else torch.as_tensor(x)
    # choose a sane float dtype
    ...
    # bounds for the increment η so that x+η stays inside [lo, hi]
    a = lo_t - x_t
    b = hi_t - x_t

    # broadcast everything to a common shape
    # scale: if scalar sigma -> expand; else broadcast
    scale0 = sigma_t if sigma_t.ndim > 0 else torch.ones_like(x_t) * sigma_t
    x_bt, a_bt, b_bt, scale_bt = torch.broadcast_tensors(x_t, a, b, scale0)

    # build TN around zero (increment), then shift by x
    loc = torch.zeros_like(x_bt)
    tn = TruncatedNormal(loc=loc, scale=scale_bt.clamp_min(1e-8), a=a_bt, b=b_bt)
    eta = tn.rsample()                      # same shape as broadcasted tensors
    x_new_t = x_bt + eta

    # (paranoia) clamp to bounds in case of tiny num. error
    lo_bt, hi_bt = torch.broadcast_tensors(lo_t, hi_t)
    x_new_t = x_new_t.clamp(min=lo_bt, max=hi_bt)

    # --- return in the same type as input ---
    if x_is_numpy:
        return x_new_t.cpu().numpy().astype(x.dtype, copy=False)
    else:
        return x_new_t.to(dtype=x_t.dtype, device=device)