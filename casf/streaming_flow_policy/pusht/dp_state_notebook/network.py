...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = x[:, None] * emb[None, :] # (batch_size, 128)
        emb = torch.cat((emb.sin(), emb.cos()), dim=-1) # (batch_size, 128+128) = (batch_size, 256)
        # emb.shape == (batch_size, self.dim)
        return emb
...
class ConditionalUnet1D(nn.Module):
    def __init__(self,
        ...
        diffusion_step_encoder = nn.Sequential(
            SinusoidalPosEmb(dsed), #  (batch_size, dsed)
    ...
    def forward(self,
        ...
        global_feature = self.diffusion_step_encoder(timesteps) # (batch_size, dsed)
        # global_feature only from timesteps, in the shape of (B, dsed=256)
        ...
        if global_cond is not None:
            ...
            # global_feature -- (B, 256) <cat> (B, 16*2) --> (B, 256+32=288)
            
        x = sample # (B, 2=action_dim, 1)
        h = []
